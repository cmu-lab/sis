//マルチエージェントシミュレータ レベル6
// レベル4　+ 食べ物の発生場所の限定 + 2種族
// by Hajime Nobuhara
...
//人間の配列を準備
Human_class[] Human = new Human_class[1000];
...
void setup() {
  ...
  for(int i=0; i < Max_Human; i++){
     ...
     int k7 = int(random(2)); //0の場合=丸顔種族 1の場合=四角顔種族　
     Human[i] = new Human_class(k1,k2,k3,k4,k5,k6,k7);
  }
 ...
 for(int j=0; j < Max_Human; j++){
   ...
   for(int j=0; j < Max_Human; j++){
    if(Human[j].type == 1){
      for(int k=0; k < Max_Human; k++){
        if((Human[j].brood != Human[k].brood)&&(Human[k].type==1)){
          if(abs(Human[j].xpos - Human[k].xpos) < 20){
            if(abs(Human[j].ypos - Human[k].ypos) < 20){
             fill(10,255,255,70);
             ellipse(Human[j].xpos, Human[j].ypos, 60, 60);
             Human[k].hp = Human[k].hp - int(random(50));
             
               if(Human[j].hp < 0){
                 Human[k].type = 0;  //もし体力が0になったら、状態を死に変更
                 MAP[Human[k].xpos][Human[k].ypos] = 0;  //状態管理の配列から消去
               }
            }
          }
        }
  ...
  for(int j=0; j < Max_Human; j++){
     if(Human[j].hp > 2000){
       ...
       Human[j].hp = Human[j].hp - 1000;
       
       int k1 = 1; //生きている人間の状態を1、死んでいる場合は 0
       int k2 = (Human[j].xpos + 15 + width) % width; //登場する初期 x座標
       int k3 = (Human[j].ypos + 15 + height) % height; //登場する初期 y座標
       int k4 = int(random(5)); //人間の動作方向をランダムにセット
       int k5 = int(random(5)); //人間の体内時計を0～4の間でセット
       int k6 = 1000; //200~400の範囲で体力をセット
       
       Human[Max_Human] = new Human_class(k1,k2,k3,k4,k5,k6,Human[j].brood);
...
class Human_class
{
  ...
  Human_class(int c, int xp, int yp, int dirt, int t, int h, int b) {
    xpos = xp;
    ypos = yp;
    type = c;
    direction = dirt;
